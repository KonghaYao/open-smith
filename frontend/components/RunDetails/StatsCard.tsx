...
export const StatsCard = (props: { run: RunRecord }) => {
    const duration = formatDuration(props.run.start_time, props.run.end_time);
    const inputSize = props.run.inputs ? new Blob([props.run.inputs]).size : 0;
    const outputSize = props.run.outputs
        ? new Blob([props.run.outputs]).size
        : 0;
